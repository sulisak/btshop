...
// add new for owner QR -----------------
// 	function Addqr()
//     {

// if(isset($_FILES["owner_qr"]["name"]) && $_FILES["owner_qr"]["name"] != ''){

// if(!file_exists("pic/owner_qr/".$_SESSION['store_id'])){
// 	mkdir("pic/owner_qr/".$_SESSION['store_id'],0777,true);
// }

//     $upload = move_uploaded_file($_FILES["owner_qr"]["tmp_name"],"pic/owner_qr/".$_SESSION['store_id']."/".time().md5($_FILES["owner_qr"]["name"]).'.jpg');

//     $data['owner_qr'] = 'pic/owner_qr/'.$_SESSION['store_id'].'/'.time().md5($_FILES["owner_qr"]["name"]).'.jpg';
...
function Addqr()
{
    ...
    if(isset($_FILES["owner_qr"]["name"]) && $_FILES["owner_qr"]["name"] != '') {
        ...
        $dir = "pic/owner_qr/".$_SESSION['store_id'];
        if(!file_exists($dir)){
            mkdir($dir, 0777, true);
        }

        $filename = $dir.'/'.time().md5($_FILES["owner_qr"]["name"]).'.jpg';

        if(move_uploaded_file($_FILES["owner_qr"]["tmp_name"], $filename)){
            $data['owner_qr'] = $filename;
        }
    }

    // Call model function to update only the fields present
    $success = $this->store_brand_model->Addimg($data);

    if($success){
        echo json_encode(['status' => 'success', 'path' => isset($data['owner_qr']) ? $data['owner_qr'] : '']);
    } else {
